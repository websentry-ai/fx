...
fn executeWorkspaceToolCallInner(
    ...
    // Unbound fork: the browser workspace also advertises fx's skill tool, which
    // needs no runtime backend, exactly as on native.
    const is_skill = spec.executor_kind == .skill;
