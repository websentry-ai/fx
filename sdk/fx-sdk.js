...
function createRuntime(options) {
  ...
  // Unbound fork: back the WASI read calls with an in-memory FS when the host
  // supplies files, so skill discovery can scan real directories.
  const memfs = options.files ? createMemFs({ root: options.workspaceRoot ?? "/workspace", files: options.files }) : null;
  const wasiFs = memfs
    ? createWasiFs(memfs, { bytes, view: () => new DataView(memory().buffer) })
    : null;
  const wasi = {
    ...
    path_open: (fd, df, p, pl, of, rb, ri, ff, out) => (wasiFs ? wasiFs.path_open(fd, df, p, pl, of, rb, ri, ff, out) : 52),
