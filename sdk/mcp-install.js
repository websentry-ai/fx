...
function remote(name, url, transport, headers) {
  if (!/^https:\/\//.test(url)) throw new Error("A remote server must use an https:// URL.");
  new URL(url);
...
function npmServer(name, command, args, env) {
  ...
  return {
    ...
    kind: "npm",
    pkg,
    version: (at === -1 ? "" : spec.slice(at + 1)) || "latest",
    args: rest,
    env,
  };
}
...
function fromJson(name, text) {
  ...
  const strings = (value) => Object.fromEntries(Object.entries(value ?? {}).map(([k, v]) => [k, String(v)]));
  if (typeof config.url === "string") {
    return remote(name, config.url, config.type === "sse" ? "sse" : "http", strings(config.headers));
  }
  if (typeof config.command === "string") {
    const args = Array.isArray(config.args) ? config.args.map(String) : [];
    return npmServer(name, config.command, args, strings(config.env));
  }
