...
try {
  ...
  assert.equal(manifest.repository.url, "git+https://github.com/websentry-ai/fx.git");
  assert.equal(manifest.homepage, "https://github.com/websentry-ai/fx/tree/main/sdk#readme");
  ...
  await assert.rejects(access(join(packageDir, "NOTICE.unbound")));
  ...
  const probe = spawnSync(process.execPath, ["--input-type=module", "--eval", `
    ...
    Client.prototype.listTools = async (params, options) => {
    ...
    const expireToolsList = () => {
      ...
      assert.equal(stalledSignal?.aborted, false, "tools/list signal started aborted");
      browserMcpTimers.values().next().value();
      assert.equal(stalledSignal.aborted, true, "tools/list timeout did not abort its request");
    };
    try {
      ...
      let closes = 0;
      Client.prototype.connect = () => new Promise(() => {});
      Client.prototype.close = async () => {
        closes += 1;
      };
      const hung = createBrowserMcp();
      hung.setServers([{ ...stalled, id: "hung", name: "hung", url: "https://hung.example.test/mcp" }]);
      const hungConnection = hung.connect();
      await Promise.resolve();
      assert.equal(browserMcpTimers.size, 1, "connect did not install its 90-second timeout");
      browserMcpTimers.values().next().value();
      assert.deepEqual(await hungConnection, {
        tools: [],
        failures: [{ server: "hung", message: "hung did not start within 90 seconds." }],
      });
      assert.equal(closes, 1, "connect timeout did not close its client");
    } finally {
      mcp.reset(stalled);
