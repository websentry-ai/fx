...
assert.match(failure("add --transport http secret https://token@example.com/mcp"), /header/);
assert.match(failure("add bad-env -e bad-name=value -- npx -y package"), /environment variable/);
assert.match(failure(`add-json bad-env '{"command":"npx","args":["-y","package"],"env":[]}'`), /JSON object/);
